...
export function ContractorSignAgreement() {
  ...
  const businessType = getStorageItem("business_type");
  ...
  const saveSignature = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (businessType !== "Clinic-Based Practice") {
          navigate("/payment");
        } else {
          navigate("/");
        }
      } else {
        toast.error(res.data.message || "Failed to save signature");
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl text-base mx-auto mt-4 px-6  md:px-10 lg:px-6">
      ...
      <p className="mb-4">
        ...
        agreement (the "<strong>Agreement</strong>") made effective as of [{today}]
        (“<strong>Effective Date</strong>”) reached between Last Minute Wellness
        Inc. (“<strong>Last Minute Wellness</strong>“) and{" "}
        <strong>[{businessName}]</strong> (the "<strong>Contractor</strong>",
        and with Last Minute Wellness, the “<strong>parties</strong>” and “
        <strong>party</strong>” means any one of them), the terms and conditions
      ...
      <div className="mt-8">
        ...
        <div className="mb-6">
          <p className="font-semibold">
            ACCEPTED AND AGREED TO AS OF THIS [{today}]
          </p>
          <p>[{businessName}]</p>
          <p>By:</p>
          <p>Name:</p>
